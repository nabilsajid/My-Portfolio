...
import { getHomeContent, getSkills } from "@/lib/db";
import { 
  Video, 
  MonitorPlay, 
  Image as ImageIcon, 
...
const AboutSection = () => {
  ...
  // Provide a default list of software if the database is empty
  const defaultSkills = [
    { name: "Premiere Pro" }, 
    { name: "After Effects" }, 
    { name: "Photoshop" }, 
        ...
        {skills.map((skill: any) => (
          ...
          >
            ...
            <div className="relative z-10 drop-shadow-md group-hover:drop-shadow-[0_0_15px_rgba(var(--primary),0.5)] transition-all duration-300">
              {skillIcons[skill.name] || <div className="w-12 h-12 rounded-full bg-primary/20" />}
            </div>
