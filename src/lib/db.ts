...
export async function updateHomeContent(id: number | undefined, data: any) {
  // If there's no ID, assume it's the first row (id: 1)
  const rowId = id || 1;
  const { data: result, error } = await supabase
    ...
    .upsert({
      ...
      happy_clients: data.happy_clients || '30+',
      years_experience: data.years_experience || '5+',
      views_generated: data.views_generated || '10M+',
    })
    .select()
    .single();
...
export const getClientLogos = async () => {
  const { data, error } = await supabase.from('client_logos').select('*').order('order_index', { ascending: true }).order('created_at', { ascending: false });
  if (error) throw error;
  return data;
};

export const addClientLogo = async (logo: Omit<any, 'id' | 'created_at'>) => {
  const { data, error } = await supabase.from('client_logos').insert([logo]).select().single();
  if (error) throw error;
  return data;
};

export const updateClientLogo = async (id: string, updates: Partial<any>) => {
  const { data, error } = await supabase.from('client_logos').update(updates).eq('id', id).select().single();
  if (error) throw error;
  return data;
};
