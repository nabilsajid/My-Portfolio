...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { name: "Home & About", path: "/admin/home", icon: Settings },
    { name: "Video Projects", path: "/admin/projects?category=video", icon: Video },
    { name: "Photography", path: "/admin/projects?category=photography", icon: Camera },
    { name: "Posters", path: "/admin/projects?category=poster", icon: Layers },
    { name: "Achievements", path: "/admin/achievements", icon: Briefcase },
    { name: "Skills", path: "/admin/skills", icon: Code },
    { name: "Experience", path: "/admin/experience", icon: ImageIcon },
    { name: "Pricing", path: "/admin/pricing", icon: CreditCard },
    { name: "Stats Counter", path: "/admin/stats", icon: BarChart },
      ...
      <div className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent">
        <nav className="px-4 space-y-1.5">
          {navItems.map((item) => {
            ...
            const isActive = location.pathname === item.path || (location.pathname === "/admin/projects" && location.search.includes(item.path.split('?')[1]));
            ...
            return (
              ...
              >
                ...
                <div className={`relative z-10 p-1.5 rounded-lg transition-colors duration-300 ${isActive ? 'bg-primary/20' : 'bg-transparent group-hover:bg-card/50'}`}>
