...
export async function addProject(project: any) {
  const { data, error } = await supabase
    .from('projects')
    .insert([{ title: project.title, category: project.category, image_url: project.image_url, label: project.label || null }])
    .select()
    .single();
...
export async function updateProject(id: number, project: any) {
  const { data, error } = await supabase
    .from('projects')
    .update({ title: project.title, category: project.category, image_url: project.image_url, label: project.label || null })
    .eq('id', id)
    .select()
    .single();
